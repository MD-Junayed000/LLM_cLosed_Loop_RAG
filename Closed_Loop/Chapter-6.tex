...
First, the probes train stably and learn genuine hallucination-relevant patterns on RAGTruth—both CEV and IAV converge smoothly, and validation metrics confirm generalization to the held-out split.

Second, out-of-domain transfer is real but model-dependent. Qwen3-8B and LLaMA-3-8B-Instruct preserve their discriminative direction on HaluEval. Mistral-7B-Instruct-v0.2 retains strong discrimination but with inverted polarity—a fascinating finding that underscores how distribution-sensitive these probes can be.

Third, calibration is not optional. Because the downstream controller makes absolute-threshold decisions, the probes need to output probabilities that actually correspond to empirical risk levels. Temperature scaling provides this, and probability-level fusion keeps the two signals independent while combining their strengths.

The bottom line: internal-state probes work, but they are not plug-and-play across domains. Validate before you deploy.
