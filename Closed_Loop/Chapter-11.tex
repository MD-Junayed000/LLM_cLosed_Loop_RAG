...
The obvious first question: why not just use the residual stream and skip the FFN intermediate entirely? We tried it. A CEV-only probe captures context-integration signals and roughly matches what earlier hidden-state probing papers do. It works—but not well enough.

The conceptual gap is clear: when hallucination arises from parametric-memory intrusion (the model injecting something ``from memory'' that contradicts the evidence), the most direct signal lives inside the FFN before it gets projected back into the residual stream. CEV sees the aftermath, not the source.

\textbf{Verdict:} useful as a baseline, incomplete as a final system.
...
We experimented with letting the fusion weight collapse freely during optimization. The risk is overfitting: if the search pushes $w$ all the way to 0 or 1 on a particular validation fold, you effectively lose one of the two signals. The system degrades to single-branch behavior—exactly what we are trying to avoid.
...
Instead of taking the last token's activation, what if we average across all generated tokens? We tried this too. The problem is dilution. Not every token position carries the same hallucination-relevant information—the earlier tokens in a generated response are often just warming up, contextually. The signal concentrates near the end, where the model has committed to its final claims. Averaging washes this out.
...
Training with hard one-hot targets makes the probe overconfident—it learns to output probabilities near 0 or 1 even when the evidence is ambiguous. That kills calibration. And calibration matters here, because the controller makes threshold-based decisions on absolute probability values. Label smoothing ($\varepsilon = 0.05$) gently discourages extreme confidence.

\textbf{Verdict:} label smoothing is not optional when the downstream consumer needs calibrated probabilities.

\subsection{Learned REINFORCE Controller}

We did attempt a learned policy—REINFORCE-style, with the probe scores as state and the controller actions as the policy's output. It did not work well. The rollout environment (100 SQuAD queries) gave sparse, noisy rewards. The policy kept collapsing toward a single action rather than learning meaningful distinctions. The deterministic threshold controller was more reliable from day one.

\textbf{Verdict:} the threshold controller wins for now. A learned policy might work with larger rollout budgets, denser rewards, and perhaps imitation learning from successful controller traces—but that is future work, not this thesis.

%% -------------------------------------------------
\section{Why the Final Configuration Works Best}
\label{sec:why_best}

Each design choice in the final system exists because a specific alternative failed. Table~\ref{tab:design_choices} maps the correspondence.
